...
from src.imported_transformer_model import HfEmilyalsentzerBioClinicalbert
...
def test_imported_transformer_model_emits_sequence_and_pooled_outputs() -> None:
    ...
    model = HfEmilyalsentzerBioClinicalbert(session_factory=_FakeSession, seq_length=4)
...
class _FakeSessionWithoutPooler(_FakeSession):
    def get_outputs(self):
        return [type("Out", (), {"name": "last_hidden_state"})()]

    def run(self, outputs, feed_dict):
        self.last_feed_dict = feed_dict
        seq_length = len(feed_dict["input_ids"][0])
        return [[[[float(i)] * 768 for i in range(seq_length)]]]


def test_imported_transformer_model_derives_pooled_embedding_when_missing() -> None:
    tokens = type("Sig", (), {"value": [1, 2, 3, 4]})()
    mask = type("Sig", (), {"value": [1, 1, 1, 1]})()
    model = HfEmilyalsentzerBioClinicalbert(session_factory=_FakeSessionWithoutPooler, seq_length=4)
    model.set_inputs({"token_ids": tokens, "attention_mask": mask})
    model.advance_window(0.0, 0.2)
    outputs = model.get_outputs()
    assert len(outputs["pooled_embedding"].value[0]) == 768
