...
class BioimageHylfmNetStat(BioModule):
    ...
    def outputs(self) -> dict[str, SignalSpec]:
        return {
            ...
            self.summary_port: SignalSpec.record(
                schema={"mean_value": "float", "voxel_count": "int"},
    ...
    def advance_window(self, start: float, end: float) -> None:
        ...
        self._outputs = {
            self.output_port: _make_signal(source=source, name=self.output_port, value=volume, emitted_at=t, spec=self.outputs().get(self.output_port) if 'self' in locals() else None),
