...
"""Imported BioImage ONNX wrapper for NucleiSegmentationBoundaryModel."""
...
class BioimageNucleisegmentationboundarymodel(BioModule):
    """Run the imported nuclei-segmentation ONNX graph as a BioModule."""

    def __init__(
        self,
        model_path: str = "artifacts/model.onnx",
        input_port: str = "image",
        output_port: str = "segmentation_logits",
        summary_port: str = "prediction_summary",
        model_input_name: Optional[str] = None,
        model_output_name: Optional[str] = None,
        input_shape: Sequence[int] = (1, 1, 256, 256),
        output_shape: Sequence[int] = (1, 2, 256, 256),
    ...
    def outputs(self) -> dict[str, SignalSpec]:
        return {
            ...
            self.summary_port: SignalSpec.record(
                schema={"mean_logit": "float", "pixel_count": "int"},
    ...
    def advance_window(self, start: float, end: float) -> None:
        ...
        pixel_count = self.output_shape[-1] * self.output_shape[-2] if len(self.output_shape) >= 2 else len(flat)
        ...
        self._outputs = {
            ...
            self.summary_port: _make_signal(source=source, name=self.summary_port, value={
                    "mean_logit": (sum(flat) / len(flat)) if flat else 0.0,
                    "pixel_count": int(pixel_count),
                }, emitted_at=t, spec=self.outputs().get(self.summary_port) if 'self' in locals() else None),
        }
