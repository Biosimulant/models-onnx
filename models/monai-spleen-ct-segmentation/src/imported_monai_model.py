...
"""Imported MONAI ONNX wrapper for Spleen CT Segmentation."""
...
class MonaiSpleenCtSegmentation(BioModule):
    """Run the imported spleen-segmentation ONNX graph as a BioModule."""
    ...
    def __init__(
        ...
        output_shape: Sequence[int] = (1, 2, 96, 96, 96),
    ...
    def advance_to(self, t: float) -> None:
        ...
        self._outputs = {
            self.output_port: BioSignal(
                ...
                value=prediction,
                time=t,
                metadata=SignalMetadata(description="Spleen CT segmentation logits", dtype="float32", shape=self.output_shape, kind="state"),
            ),
            self.summary_port: BioSignal(
                ...
                time=t,
                metadata=SignalMetadata(description="Summary statistics over the latest spleen-segmentation output tensor", kind="metric"),
            ),
        }
