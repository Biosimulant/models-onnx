...
"""Imported MONAI ONNX wrapper for BRATS MRI Segmentation."""
...
class MonaiBratsMriSegmentation(BioModule):
    """Run the imported BRATS segmentation ONNX graph as a BioModule."""
    ...
    def __init__(
        ...
        input_shape: Sequence[int] = (1, 4, 128, 128, 128),
        output_shape: Sequence[int] = (1, 3, 128, 128, 128),
    ...
    def advance_to(self, t: float) -> None:
        ...
        self._outputs = {
            self.output_port: BioSignal(
                ...
                value=prediction,
                time=t,
                metadata=SignalMetadata(description="BRATS tumor-region logits", dtype="float32", shape=self.output_shape, kind="state"),
            ),
            self.summary_port: BioSignal(
                ...
                time=t,
                metadata=SignalMetadata(description="Summary statistics over the latest BRATS segmentation output tensor", kind="metric"),
            ),
        }
