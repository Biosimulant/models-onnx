...
"""Imported transformer ONNX wrapper for esm2_t12_35M_UR50D."""
...
class HfFacebookEsm2T1235mUr50d(BioModule):
    """Run the imported ESM-2 t12 ONNX export behind the BioModule interface."""
    ...
    def advance_to(self, t: float) -> None:
        ...
        sequence_embeddings = result[0] if result else [[[0.0] * 480 for _ in range(self.seq_length)]]
        if len(result) > 1:
            ...
            pooled_embedding = [first_token] if first_token else [[0.0] * 480]
        ...
        self._outputs = {
            self.sequence_port: BioSignal(
                ...
                metadata=SignalMetadata(
                    description="esm2_t12_35M_UR50D token-level embedding tensor",
                    dtype="float32",
                    shape=(1, self.seq_length, 480),
                    kind="state",
                ),
            ),
            self.pooled_port: BioSignal(
                ...
                time=t,
                metadata=SignalMetadata(
                    description="esm2_t12_35M_UR50D pooled embedding vector",
                    dtype="float32",
                    shape=(1, 480),
                    kind="state",
                ),
